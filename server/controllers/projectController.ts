...
export const makeRevision = async (req: Request,res:Response) => {
    ...
    try {
        ...
        if(user.credits<5){
            return res.status(403).json({message:'Insufficient credits please add credits to make revisions'});
        }
...
export const rollbackToVersion = async (req: Request,res:Response) => {
   ...
   try {
    ...
    const version=await prisma.version.find((version)=>version.id===versionId);
    ...
    await prisma.conversation.create({
        data:{
            role:'assistant',
            content:`i've rolled back to your website to selected version. You can preview it `,
            projectId
        }
    })
...
export const saveProjectCode = async (req: Request,res:Response) => {
   ...
   try {
    ...
    if(!code){
        return res.status(400).json({message:'Code is required to save project'});
    }
